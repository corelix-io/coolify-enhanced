<!DOCTYPE html>
@php
    $ceThemeSlug = class_exists('CorelixIo\Platform\Models\EnhancedUiSettings')
        ? \CorelixIo\Platform\Models\EnhancedUiSettings::getActiveTheme()
        : null;
@endphp
...
<head>
    ...
    @php
        $wlBrand = 'Coolify';
        $wlDesc = 'An open-source & self-hostable Heroku / Netlify / Vercel alternative';
        $wlWebsite = 'https://coolify.io';
        $wlTwitter = '@coolifyio';
        $wlOgImage = null;
        if (function_exists('whitelabelEnabled') && whitelabelEnabled()
            && class_exists('\CorelixIo\Platform\Services\WhitelabelConfig')) {
            $wlBrand = function_exists('brandName') ? brandName() : $wlBrand;
            $wlDesc = \CorelixIo\Platform\Services\WhitelabelConfig::brandDescription() ?: $wlDesc;
            $wlWebsite = \CorelixIo\Platform\Services\WhitelabelConfig::websiteUrl() ?: $wlWebsite;
            $wlTwitter = \CorelixIo\Platform\Services\WhitelabelConfig::twitterHandle() ?: $wlTwitter;
            $wlOgImage = \CorelixIo\Platform\Services\WhitelabelConfig::ogImageUrl();
        }
    @endphp
    <meta name="Description" content="{{ $wlBrand }}: {{ $wlDesc }}" />
    <meta name="viewport" content="width=device-width,initial-scale=1" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:site" content="{{ $wlTwitter }}" />
    <meta name="twitter:title" content="{{ $wlBrand }}" />
    <meta name="twitter:description" content="{{ $wlDesc }}." />
    <meta name="twitter:image" content="{{ $wlOgImage ?? 'https://cdn.coollabs.io/assets/coolify/og-image.png' }}" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="{{ $wlWebsite }}" />
    <meta property="og:title" content="{{ $wlBrand }}" />
    <meta property="og:description" content="{{ $wlDesc }}." />
    <meta property="og:site_name" content="{{ $wlBrand }}" />
    <meta property="og:image" content="{{ $wlOgImage ?? 'https://cdn.coollabs.io/assets/coolify/og-image.png' }}" />
    ...
    @if(function_exists('whitelabelEnabled') && whitelabelEnabled() && function_exists('brandFaviconUrl'))
    ...
    <script>window.__FEATURES__ = @json(\CorelixIo\Platform\Support\Feature::frontendFlags());</script>
    ...
    @endauth
    {{-- Corelix Enhanced: Multi-theme support (CSS + JS) --}}
    @php
        $ceThemeConfig = $ceThemeSlug
            ? config("corelix-platform.ui_theme.themes.{$ceThemeSlug}")
            : null;
    @endphp
    @if($ceThemeConfig)
    <link rel="stylesheet" href="{{ asset('vendor/corelix-platform/' . $ceThemeConfig['css']) }}">
    ...
    <script defer src="{{ asset('vendor/corelix-platform/' . $ceThemeConfig['js']) }}"></script>
    @endif
    @endif
</head>
@section('body')

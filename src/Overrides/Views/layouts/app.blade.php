...
@section('body')
    ...
    @auth
        <div x-data="{
            ...
            {{-- [CORELIX ENHANCED: Mobile top bar — brand name/logo from whitelabel service when whitelabeling enabled] --}}
            @php
                $ceMobileWlEnabled = function_exists('whitelabelEnabled') && whitelabelEnabled();
                $ceMobileBrandName = $ceMobileWlEnabled && function_exists('brandName') ? brandName() : 'Coolify';
                $ceMobileBrandLogoPath = $ceMobileWlEnabled
                    ? config('corelix-platform.whitelabel.brand_logo')
                    : null;
                $ceMobileBrandLogoUrl = ($ceMobileBrandLogoPath && file_exists(public_path($ceMobileBrandLogoPath)))
                    ? asset($ceMobileBrandLogoPath)
                    : null;
            @endphp
            <div
                ...
                <div class="flex items-center gap-3 flex-shrink-0">
                    <a href="/"
                        class="text-xl font-bold tracking-wide dark:text-white hover:opacity-80 transition-opacity flex items-center min-h-[1.5rem]">
                        @if($ceMobileBrandLogoUrl)
                            <img src="{{ $ceMobileBrandLogoUrl }}"
                                 alt="{{ $ceMobileBrandName }}"
                                 class="h-6 w-auto max-w-[120px] object-contain object-left"
                                 onerror="this.style.display='none'; this.nextElementSibling.style.display='inline';">
                            <span style="display:none">{{ $ceMobileBrandName }}</span>
                        @else
                            {{ $ceMobileBrandName }}
                        @endif
                    </a>
                    {{-- [CORELIX ENHANCED: End mobile brand block] --}}

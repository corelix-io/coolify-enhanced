        ...
        @if ($current_step === 'type')
            <div x-init="window.addEventListener('scroll', () => isSticky = window.pageYOffset > 100)"
                ...
                <div class="flex gap-2 items-start">
                    ...
                    {{-- Corelix Enhanced: Source Filter Dropdown --}}
                    ...
                    <div class="grid justify-start grid-cols-1 gap-4 text-left xl:grid-cols-3">
                        <template x-for="service in filteredServices" :key="service.name">
                            <div class="relative" x-on:click="setType('one-click-service-' + service.name, service._ignored)"
                                ...
                                <x-resource-view>
                                    ...
                                    <x-slot:logo>
                                        <template x-if="service.logo">
                                            {{-- [CORELIX ENHANCED: fallback logo uses brandLogoUrl() so whitelabeled instances show their own logo instead of the Coolify logo] --}}
                                            <img class="w-full h-full p-2 transition-all duration-200 dark:bg-white/10 bg-black/10 object-contain"
                                                :src='service.logo'
                                                x-on:error.window="$event.target.src = service.logo_github_url"
                                                onerror="this.onerror=null; this.src=this.getAttribute('data-fallback');"
                                                x-on:error="$event.target.src = '{{ function_exists('brandLogoUrl') ? brandLogoUrl() : asset('coolify-logo.svg') }}'"
                                                :data-fallback='service.logo_github_url' />
                                        </template>
                                    </x-slot:logo>
                                </x-resource-view>
                                {{-- Corelix Enhanced: Custom template source label --}}
                                ...
                                {{-- Corelix Enhanced: Untested/ignored template warning badge --}}
                                ...
                                <template x-if="service.amd_only">
                                    <div class="absolute right-10 group"
                                        :style="(service._source && service._ignored) ? 'top: 2.25rem' : ((service._source || service._ignored) ? 'top: 1.25rem' : 'top: 0.5rem')">
                                        <span
            ...
            <script>
                ...
                function searchResources() {
                    return {
                        search: '',
                        selectedCategory: '',
                        selectedSource: '', // Corelix Platform: source filter
                        categories: [],
                        sources: [], // Corelix Platform: unique custom source names
                        loading: false,
                        isSticky: false,
                        selecting: false,
                        services: [],
                        ...
                        setType(type, ignored = false) {
                            ...
                            // Corelix Platform: confirmation for untested/ignored templates
                            if (ignored) {
                        ...
                        async loadResources() {
                            ...
                            // Corelix Platform: extract unique custom source names
                        ...
                        filterAndSort(items, isSort = true) {
                            ...
                            // Corelix Platform: Filter by source if selected

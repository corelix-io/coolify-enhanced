    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        <div class="sub-menu-wrapper">
            ...
            @if (config('corelix-platform.enabled', false) && config('corelix-platform.dns_provider_management.enabled', false))
                <a class="sub-menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"
                    href="{{ route('project.application.domains', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'application_uuid' => $application->uuid]) }}"><span class="menu-item-label">Domains</span></a>
            @endif
            <a class="sub-menu-item" {{ wireNavigate() }} wire:current.exact="menu-item-active"

                ...
                <div class="flex flex-col gap-2">
                    <div class="flex gap-2">
                        <x-forms.select x-bind:disabled="shouldDisable()" wire:model.live="buildPack" label="Build Pack"
                            ...
                            {{-- Corelix Enhanced: Additional build types (Cloud Native Buildpacks). Railpack is native upstream. --}}
                        ...
                        @if (
                            ...
                            @php
                                $hasNonDatabaseService = collect(data_get($parsedServices, 'services', []))
                                    ->contains(fn($service) => !isDatabaseImageEnhanced(data_get($service, 'image'), $service));
                            @endphp
                            @if ($hasNonDatabaseService)
                                <h3 class="pt-6">Domains</h3>
                            @endif
                            @foreach (data_get($parsedServices, 'services') as $serviceName => $service)
                                @if (!isDatabaseImageEnhanced(data_get($service, 'image'), $service))

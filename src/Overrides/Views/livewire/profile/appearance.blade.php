<div>
    ...
    <div x-data="{
        ...
        {{-- Corelix Enhanced: discoverable link to the instance-wide UI Theme settings (owner/admin only).
             Upstream's Appearance controls are per-user/per-browser; the Enhanced UI Theme is an
             instance-wide setting, so it lives on its own page and is only surfaced to admins here. --}}
        @if (config('corelix-platform.enabled') && function_exists('isInstanceAdmin') && isInstanceAdmin())
            <section class="space-y-1.5">
                <h2>UI Theme</h2>

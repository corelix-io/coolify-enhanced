<nav class="flex flex-col flex-1 bg-white border-r dark:border-coolgray-200 border-neutral-300 dark:bg-base"
    ...
    {{-- [CORELIX ENHANCED: upstream v4.1.x collapsible-sidebar wrappers preserved] --}}
    <div class="flex pt-4 pb-4 pl-2 pr-3 items-start gap-3"
        ...
        <div class="flex min-w-0 flex-1 flex-col" :class="collapsed && 'lg:hidden'">
            {{-- [CORELIX ENHANCED: Brand name / logo — uses whitelabel service when whitelabeling is enabled] --}}
            @php
                $ceWlEnabled = function_exists('whitelabelEnabled') && whitelabelEnabled();
                $ceBrandName = $ceWlEnabled && function_exists('brandName') ? brandName() : 'Coolify';
                $ceBrandLogoPath = $ceWlEnabled
                    ? config('corelix-platform.whitelabel.brand_logo')
                    : null;
                $ceBrandLogoUrl = ($ceBrandLogoPath && file_exists(public_path($ceBrandLogoPath)))
                    ? asset($ceBrandLogoPath)
                    : null;
            @endphp
            <a href="/" {{ wireNavigate() }} class="text-2xl font-bold tracking-tight dark:text-white hover:opacity-80 transition-opacity flex items-center min-h-[2rem]">
                @if($ceBrandLogoUrl)
                    <img src="{{ $ceBrandLogoUrl }}"
                         alt="{{ $ceBrandName }}"
                         class="h-8 w-auto max-w-[140px] object-contain object-left"
                         onerror="this.style.display='none'; this.nextElementSibling.style.display='inline';">
                    <span style="display:none">{{ $ceBrandName }}</span>
                @else
                    {{ $ceBrandName }}
                @endif
            </a>
            {{-- [CORELIX ENHANCED: End brand block] --}}
            <x-version />
        </div>
        ...
        <li class="flex-1 overflow-x-hidden">
            <ul role="list" class="flex flex-col h-full space-y-1.5">
                @if (isSubscribed() || !isCloud())
                    ...
                    {{-- [CORELIX ENHANCED: Sponsor/support link — hidden when whitelabeling disables sponsorship; replaced with custom support URL when configured. Collapsed-sidebar class hooks from upstream v4.1.x preserved on label spans] --}}
                    @php
                        $ceSponsorHidden = $ceWlEnabled
                            && class_exists('\CorelixIo\Platform\Services\WhitelabelConfig')
                            && \CorelixIo\Platform\Services\WhitelabelConfig::hideSponsorships();
                        $ceSupportUrl = $ceWlEnabled
                            && class_exists('\CorelixIo\Platform\Services\WhitelabelConfig')
                            ? \CorelixIo\Platform\Services\WhitelabelConfig::supportUrl()
                            : null;
                    @endphp
                    @unless($ceSponsorHidden)
                        <li>
                            @if($ceSupportUrl)
                                <a title="Support" class="menu-item" href="{{ $ceSupportUrl }}" target="_blank">
                                    ...
                                    <span class="menu-item-label" :class="collapsed && 'lg:hidden'">Support</span>
                                </a>
                            @else
                    ...
                    @endunless
                    {{-- [CORELIX ENHANCED: End sponsor/support block] --}}
                @endif

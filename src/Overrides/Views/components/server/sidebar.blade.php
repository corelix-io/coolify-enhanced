<div class="sub-menu-wrapper">
    ...
    @if (config('corelix-platform.enabled', false) && $server->isFunctional())
        <a class="sub-menu-item {{ $activeMenu === 'resource-backups' ? 'menu-item-active' : '' }}" {{ wireNavigate() }}
    ...
    @else
        @if (config('corelix-platform.enabled', false) && $server->isFunctional())
            <span class="sub-menu-item opacity-50 cursor-default" title="Available in Pro"><span class="menu-item-label">Registries (Pro)</span></span>
        @endif
    @endfeature
    {{-- Corelix Enhanced: Networks sidebar item --}}
    @if (config('corelix-platform.enabled', false) && config('corelix-platform.network_management.enabled', false) && $server->isFunctional())

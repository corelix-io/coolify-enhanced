<?php
// =============================================================================
// OVERLAY: Modified version of Coolify's bootstrap/helpers/docker.php
// =============================================================================
// Changes from the original are marked with overlay comments:
//
// [PROXY ISOLATION OVERLAY] — fqdnLabelsForTraefik() and fqdnLabelsForCaddy()
//   accept optional $proxyNetwork parameter. generateLabelsApplication() resolves
//   the proxy network name and passes it to both label functions.
//   This injects traefik.docker.network and caddy_ingress_network labels for
//   proxy network isolation when CORELIX_PROXY_ISOLATION=true.
// =============================================================================
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null, ?string $proxyNetwork = null)
{
    ...
    // [PROXY ISOLATION OVERLAY] Tell Traefik which Docker network to use for routing
    // Without this label, Traefik randomly picks a network IP in multi-network setups,
    // causing intermittent 502 errors that depend on Docker's internal network ordering.
    if ($proxyNetwork) {
        ...
        try {
            ...
            if ($server) {
                if (config('corelix-platform.network_management.proxy_isolation', false)) {
                    $proxyNet = \CorelixIo\Platform\Models\ManagedNetwork::where('server_id', $server->id)
                ...
                if (! $proxyNetworkName) {
                    $proxyNetworkName = $server->isSwarm() ? 'coolify-overlay' : 'coolify';
                }
            }
        } catch (\Throwable $e) {
            // Silently fail — label generation should not break deployments
        }
    }
    // [END PROXY ISOLATION OVERLAY]

...
//   CORELIX_PROXY_ISOLATION=true. Covers Service and Application label generation.
// =============================================================================
 ...
 *
 * Stricter than validateShellSafePath: also rejects path separators, '..' sequences,
 * NUL bytes, and shell-globbing/expansion metacharacters. Intended for caller-supplied
...
/**
 * Extract inline comment from a value string, handling quoted values.
...
function get_service_templates(bool $force = false): Collection
{
    ...
    if ($force) {
        try {
            ...
            $builtIn = collect(json_decode($services))->sortKeys();
        }
    } else {
        $services = File::get(base_path('templates/'.config('constants.services.file_name')));
        $builtIn = collect(json_decode($services))->sortKeys();
    ...
    // [CUSTOM TEMPLATES OVERLAY] Merge custom templates from GitHub sources
    if (config('corelix-platform.enabled', false)) {
        try {
            $custom = \CorelixIo\Platform\Services\TemplateSourceService::getCachedCustomTemplates($builtIn);
            if ($custom->isNotEmpty()) {
                $builtIn = $builtIn->merge($custom);
            }
        } catch (\Throwable $e) {
            // Log but never break the template loading — built-in templates must always work
            \Illuminate\Support\Facades\Log::warning('Corelix Platform: Failed to load custom templates: '.$e->getMessage());
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('Corelix Platform: Failed to load ignored templates: '.$e->getMessage());
        }
                        ...
                        if (config('corelix-platform.enabled', false)
                            ...
                            try {
                                if (config('corelix-platform.network_management.proxy_isolation', false)) {
                                    $proxyNet = \CorelixIo\Platform\Models\ManagedNetwork::where('server_id', $resource->server->id)
                                        ->where('is_proxy_network', true)->where('status', 'active')->first();
                                    $proxyNetworkName = $proxyNet?->docker_network_name;
                                }

                                if (! $proxyNetworkName) {
                                    $proxyNetworkName = $resource->server->isSwarm() ? 'coolify-overlay' : 'coolify';
                                }
                            } catch (\Throwable $e) {
                ...
                if ($domains) {
                    ...
                    if ($fqdns) {
                        ...
                        if (config('corelix-platform.enabled', false)
                            ...
                            try {
                                if (config('corelix-platform.network_management.proxy_isolation', false)) {
                                    $appProxyNet = \CorelixIo\Platform\Models\ManagedNetwork::where('server_id', $server->id)
                                        ->where('is_proxy_network', true)->where('status', 'active')->first();
                                    $appProxyNetworkName = $appProxyNet?->docker_network_name;
                                }

                                if (! $appProxyNetworkName) {
                                    $appProxyNetworkName = $server->isSwarm() ? 'coolify-overlay' : 'coolify';
                                }
                            } catch (\Throwable $e) {

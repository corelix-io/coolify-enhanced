...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function decide_what_to_do()
    {
        ...
        // [CORELIX ENHANCED: Additional build types — Heroku/Paketo Cloud Native Buildpacks. Railpack is now native upstream.]
    ...
    private function deploy_pull_request()
    {
        ...
        if ($this->application->build_pack === 'railpack') {
            $this->build_railpack_image();
        // [CORELIX ENHANCED: Additional build types for PR deploys — Heroku/Paketo CNB]
        } elseif ($this->application->build_pack === 'heroku') {
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
    ...
    private function generate_compose_file()
    {
        ...
        // [CORELIX ENHANCED: persist managed-network membership in the compose
        // definition so it survives container recreation (redeploy/reboot).
        // Runtime `docker network connect` alone is ephemeral and leaves the
        // resource_networks pivot reporting a stale "connected" state. Only
        // networks that already exist on the host are injected (verified), so a
        // missing/stale network can never break `docker compose up` — the
        // post-deploy reconcile then creates+connects it and the next deploy
        // persists it. See ManagedNetworkComposeService.]
        try {
            $managedComposeNetworks = \CorelixIo\Platform\Services\ManagedNetworkComposeService::resolveNetworkNames(
                $this->application,
                $this->destination->server ?? null,
                $this->application->environment ?? null,
            );
            $injectedCompose = \CorelixIo\Platform\Services\ManagedNetworkComposeService::injectNetworks(
                $docker_compose,
                $managedComposeNetworks,
            );
            if ($injectedCompose !== null) {
                $docker_compose = $injectedCompose;
            }
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::warning('NetworkManagement: managed-network compose injection failed', [
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
    ...
    // [CORELIX ENHANCED: Cloud Native Buildpacks (Heroku/Paketo). Railpack is now native upstream and intentionally NOT overlaid here.]
        ...
        if ($this->pull_request_id === 0) {
            foreach ($this->application->nixpacks_environment_variables as $env) {
                if (! is_null($env->real_value) && $env->real_value !== '') {
                    $env_flags .= ' --env '.escapeShellValue("{$env->key}={$env->real_value}");
                }
            }
        } else {
            foreach ($this->application->nixpacks_environment_variables_preview as $env) {
    ...
    // [END CORELIX ENHANCED: Cloud Native Buildpacks]
}
